...
def test_get_new_df(spark_fixture):
    ...
    new_df = create_spark_df(spark, new_key, new_df_cols)
    day = date_regex_extract(new_key)
    gndf = get_new_df(extraction_df, existing_df, args['args']['partitioning_column'], day)
    assert new_df.schema == gndf.schema, "failed to identify new rows since latest import"
