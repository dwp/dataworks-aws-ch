  ...
  content = templatefile("bootstrap_actions/emr-setup.sh",
    {
      ...
      S3_COMMON_LOGGING_SHELL         = format("s3://%s/%s", local.config_bucket, data.terraform_remote_state.common.outputs.application_logging_common_file.s3_id)
      S3_LOGGING_SHELL                = format("s3://%s/%s", local.config_bucket, aws_s3_bucket_object.logging_script.key)
      ...
      truststore_certs                = "s3://${local.env_certificate_bucket}/ca_certificates/dataworks/dataworks_root_ca.pem,s3://${var.data_public_certificate_bucket_id}/ca_certificates/dataworks/dataworks_root_ca.pem"
      ...
      S3_CLOUDWATCH_SHELL             = format("s3://%s/%s", local.config_bucket, aws_s3_bucket_object.cloudwatch_sh.key)
