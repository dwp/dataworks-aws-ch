...
echo "${CH_LOG_LEVEL}" > /opt/emr/log_level
...
(
    ...
    function log_wrapper_message() {
        log_ch_message "$${1}" "download-scripts.sh" "$${PID}" "$${@:2}" "Running as: ,$USER"
    }

    log_wrapper_message "Downloading & install latest bootstrap scripts"
...
)  >> /var/log/dataworks-aws-ch/download-scripts.log 2>&1
